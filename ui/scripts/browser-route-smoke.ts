import { buildRoadmapSmokeHeaders } from "./route-smoke-utils";
...
const smokeUserId = process.env.SMOKE_USER_ID?.trim() ?? "local-smoke-user";
  ...
  for (const route of routes) {
    for (const width of viewports) {
      ...
      await page.setExtraHTTPHeaders(
        buildRoadmapSmokeHeaders(smokeUserId, smokeAuthToken),
      );
      ...
      try {
        ...
        consoleErrors.push(
          error instanceof Error ? error.name : "navigation_failed",
        );
      }
      ...
      const hasControlledState =
        /Your next steady step|See what is ahead|could not load|Try again|setup/i.test(
          successText,
        );
      const ok =
      ...
      if (!ok || consoleErrors.length > 0 || failedRequests.length > 0)
        failed = true;
      await page.close();
    }
  }
...
if (failed)
  throw new Error(
    "Browser route smoke failed; inspect structured diagnostics above.",
  );
